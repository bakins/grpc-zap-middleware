...
// DurationToTimeMillisField converts the duration to milliseconds and uses the key `grpc_time_ms`.
func DurationToTimeMillisField(duration time.Duration) zapcore.Field {
	return zap.Float32("grpc_time_ms", durationToMilliseconds(duration))
}
...
func DurationToDurationField(duration time.Duration) zapcore.Field {
	return zap.Duration("grpc_duration", duration)
}
