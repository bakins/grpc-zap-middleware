...
import (
	...
	"github.com/bakins/grpc-zap-middleware"
...
func (s *zapServerSuite) TestPing_WithCustomTags() {
	...
	assert.Contains(s.T(), msgs[1], `grpc_time_ms":`, "interceptor log statement should contain execution time")
}
...
func (s *zapServerOverrideSuite) TestPing_HasOverriddenDuration() {
	...
	assert.Contains(s.T(), msgs[0], `"msg": "some ping"`, "handler's message must contain user message")
	assert.NotContains(s.T(), msgs[0], "grpc_time_ms", "handler's message must not contain default duration")
	assert.NotContains(s.T(), msgs[0], "grpc_duration", "handler's message must not contain overridden duration")
	assert.Contains(s.T(), msgs[1], `"msg": "finished unary call"`, "interceptor message must contain string")
	assert.Contains(s.T(), msgs[1], `"level": "info"`, "OK error codes must be logged on info level.")
	assert.NotContains(s.T(), msgs[1], "grpc_time_ms", "interceptor message must not contain default duration")
	assert.Contains(s.T(), msgs[1], "grpc_duration", "interceptor message must contain overridden duration")
}
